  ...
  <div style="background:linear-gradient(135deg,#0d2147,#112266);border:1px solid rgba(56,139,253,.25);border-radius:var(--radius);padding:20px;margin-bottom:16px">
    ...
    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-top:16px;padding-top:16px;border-top:1px solid rgba(255,255,255,.1)">
      ${[
        ...
        (p.status==='completed'&&p.completedDate&&p.endDate)?(function(){
          const diff=(typeof daysBetween==='function')?daysBetween(p.completedDate,p.endDate):Math.round((new Date(p.endDate)-new Date(p.completedDate))/86400000);
          // diff > 0 → finished BEFORE planned end (ahead of schedule)
          // diff < 0 → finished AFTER planned end (delayed)
          // diff = 0 → on time
          if(diff>0)return['fa-check-circle','Schedule Variance','<span style="color:#3fb950">'+diff+'d ahead</span><br><span style="font-size:9px;color:rgba(255,255,255,.5);font-weight:400">Completed '+p.completedDate+'</span>'];
          if(diff<0)return['fa-exclamation-triangle','Schedule Variance','<span style="color:var(--accent-red)">'+Math.abs(diff)+'d delayed</span><br><span style="font-size:9px;color:rgba(255,255,255,.5);font-weight:400">Completed '+p.completedDate+'</span>'];
          return['fa-check','Schedule Variance','<span style="color:#3fb950">On time</span><br><span style="font-size:9px;color:rgba(255,255,255,.5);font-weight:400">Completed '+p.completedDate+'</span>'];
  ...
  <div class="grid grid-2" style="margin-bottom:16px">
    <div class="card">
      ...
      <div class="form-grid">
        ...
        <div class="form-group"><label class="form-label">Completion Date <span style="font-size:10px;color:var(--text-muted)">— set when status is completed</span></label><input class="form-input" type="date" id="dp_completed" value="${safeDate(p.completedDate)}" ${p.status==='completed'?'':'disabled style="opacity:.5;cursor:not-allowed"'}></div>
...
function saveDetailOverview(){
  ...
  const _manualCompleted=$('#dp_completed')?.value||'';
  if(p.status==='completed'){
    // Prefer any date the user typed in the Completion Date field; otherwise
    // keep the existing one, or stamp today on the first transition to completed.
    p.completedDate=_manualCompleted||p.completedDate||new Date().toISOString().slice(0,10);
  } else {
    // Reopening a project — drop the recorded completion date
    p.completedDate='';
  }
...
function renderDetailTasks(){
  ...
  <div class="card">
    <div class="section-header" style="margin-bottom:12px">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        <button class="btn btn-secondary btn-sm" onclick="exportTasksExcel('${detailProjectId}')" title="Export tasks to CSV (opens in Excel)"><i class="fas fa-file-export"></i> Export</button>
        <button class="btn btn-secondary btn-sm" onclick="taskProjectFilter='${detailProjectId}';showTaskImport()" title="Import tasks from CSV / Excel"><i class="fas fa-file-import"></i> Import CSV</button>
