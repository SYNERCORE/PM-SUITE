  ...
  wrap.innerHTML = dl + _wfEdSteps.map((s, i) => `
    <div style="border:1px solid var(--border);border-radius:8px;padding:10px;margin-bottom:8px">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
        <input class="form-input" list="wfUserList" style="flex:2;min-width:180px;height:28px;font-size:11px" placeholder="Add approver email + Enter"
          onkeydown="if(event.key==='Enter'){event.preventDefault();const v=this.value.trim().toLowerCase();if(v.includes('@')&&!_wfEdSteps[${i}].approvers.includes(v)){_wfEdSteps[${i}].approvers.push(v);_wfEdRenderSteps();}}">
