...
// Backfill Planned Hrs from durationHrs across every project so the global
// task table shows the same values as the project detail Task List.
let _phDirty=false;
_wbsProjs.forEach(pid=>{
  const hpd=(typeof _getTaskProjHPD==='function')?_getTaskProjHPD(pid):8;
  (AppState.data.tasks||[])
    .filter(t=>t.projectId===pid&&!t._deleted&&!t.milestone&&t.durationHrs>0&&(!t.plannedHrs||t.plannedHrs===0))
    .forEach(t=>{t.plannedHrs=+t.durationHrs.toFixed(2);_phDirty=true;});
});
if(_wbsDirty||_phDirty)AppState.save();
const el=$('#tasks');
// Import from Project + Export CSV only make sense for a specific project scope
const _perProjBtns = (taskProjectFilter && taskProjectFilter!=='all') ? `
<button class="btn btn-secondary btn-sm" onclick="exportTasksExcel('${taskProjectFilter}')" title="Export tasks for the selected project to CSV"><i class="fas fa-file-export"></i> Export</button>
<button class="btn btn-secondary btn-sm" onclick="showImportFromProject('${taskProjectFilter}')" title="Copy tasks from another project"><i class="fas fa-copy"></i> Import from Project</button>` : '';
...
<select class="form-select" style="width:180px;height:32px" id="taskProjSel" onchange="taskProjectFilter=this.value;renderTasks()">
<option value="all">All Projects</option>
${(AppState.data.projects||[]).map(p=>`<option value="${p.id}" ${taskProjectFilter===p.id?'selected':''}>${esc(p.id)}</option>`).join('')}
</select>
...
${_perProjBtns}
<button class="btn btn-secondary btn-sm" onclick="showTaskImport()" title="Import tasks from CSV / Excel"><i class="fas fa-file-import"></i> Import CSV</button>
...
// ── Actual vs planned finish variance (calendar days; + = late, - = early) ──
// Self-contained YMD parsing so it never depends on CPM date helpers.
function _ymdToMs(s){const m=/^(\d{4})-(\d{2})-(\d{2})/.exec(s||'');return m?Date.UTC(+m[1],+m[2]-1,+m[3]):null;}
function _taskFinishVarDays(t){const a=_ymdToMs(t&&t.actualEnd),p=_ymdToMs(t&&t.endDate);return(a===null||p===null)?null:Math.round((a-p)/86400000);}
function _varBadge(days){
  if(days===null||days===undefined||isNaN(days))return '';
  if(days===0)return '<span style="font-size:9px;color:var(--accent-green,#2e7d46)">on time</span>';
  const late=days>0;
  return `<span style="font-size:9px;color:${late?'var(--accent-red)':'var(--accent-green,#2e7d46)'}">${late?'+'+days:days}d ${late?'late':'early'}</span>`;
}
...
<div class="form-group"><label class="form-label">Actual Start <span style="font-size:10px;color:var(--text-muted)">— when it really began</span></label><input class="form-input" type="date" id="tActStart" value="${t?.actualStart||''}"></div>
<div class="form-group"><label class="form-label">Actual End <span id="tActVar" style="margin-left:4px"></span></label><input class="form-input" type="date" id="tActEnd" value="${t?.actualEnd||''}" onchange="_tActualVar()"></div>
...
const t={
  ...
  actualStart:$('#tActStart')?.value||'',   // real start — independent of CPM schedule
  actualEnd:$('#tActEnd')?.value||'',        // real finish — independent of CPM schedule
