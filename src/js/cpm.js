...
function runFullCPM(tasks, project) {
  ...
  // ── Forward Pass ──────────────────────────────────────────────
  // Track hours-within-day so fractional-duration tasks can share a working day
  // (two 0.5-day FS tasks fit into one 8-hour day; a 0.25-day pred → 0.75-day
  // succ starts same day at hour 2).
  const hpd = cal.hoursPerDay || 8;
  const durHours = (t) => getDurDays(t) * hpd;
  // Advance (date, hoursOffset) forward by `hours` on the given calendar.
  const walkHours = (date, offset, hours) => {
    if (hours <= 0) return { date, offset };
    let remaining = offset + hours;
    let d = date;
    while (remaining > hpd + 1e-9) {
      remaining -= hpd;
      d = addWorkingDays(d, 1, cal);
    }
    return { date: d, offset: remaining };
  };

  for (const id of order) {
    const t    = map.get(id);
    const preds = parsePredecessors(t.predecessors || '');
    const dHrs = durHours(t);
    ...
    for (const p of preds) {
      ...
      let constraintDate, constraintHrs = 0;
      ...
      switch (p.type) {
        case 'FS': {
          ...
          if (lag === 0 && predPartial && succSubDay) {
            ...
            constraintDate = addWorkingDays(pred._EF, lag + 1, cal);
            constraintHrs = 0;
          }
          break;
        }
        ...
        case 'FF':
          ...
          break;
        case 'SF':
          constraintDate = addWorkingDays(addWorkingDays(pred._ES, lag, cal), -Math.max(0, getDurDays(t) - 1), cal);
          break;
        default:
          constraintDate = addWorkingDays(pred._EF, lag + 1, cal);
      }

      if (es === null || constraintDate > es || (constraintDate === es && constraintHrs > startHrs)) {
        es = constraintDate;
        startHrs = constraintHrs;
      }
    }
    ...
    t._ES = es;
    t._startHrs = startHrs;
    if (dHrs <= 0) {
      t._EF = es;
      t._endHrs = startHrs;
    } else {
      const end = walkHours(es, startHrs, dHrs);
      t._EF = end.date;
      t._endHrs = end.offset;
    }
    map.set(id, t);
  }
