...
-- Helper: a generated column can't be ALTERed in place; drop then re-add.
...
ALTER TABLE projects ADD COLUMN start_date DATE GENERATED ALWAYS AS (
  CASE WHEN data->>'startDate' ~ '^\d{4}-\d{2}-\d{2}' THEN (data->>'startDate')::DATE ELSE NULL END
) STORED;
ALTER TABLE projects ADD COLUMN end_date DATE GENERATED ALWAYS AS (
  CASE WHEN data->>'endDate' ~ '^\d{4}-\d{2}-\d{2}' THEN (data->>'endDate')::DATE ELSE NULL END
) STORED;
...
ALTER TABLE warehouse_items ADD COLUMN qty_on_hand NUMERIC GENERATED ALWAYS AS (
  CASE WHEN data->>'qtyOnHand' ~ '^-?[0-9]+(\.[0-9]+)?$' THEN (data->>'qtyOnHand')::NUMERIC ELSE NULL END
) STORED;
ALTER TABLE warehouse_items ADD COLUMN reorder_level NUMERIC GENERATED ALWAYS AS (
  CASE WHEN data->>'reorderLevel' ~ '^-?[0-9]+(\.[0-9]+)?$' THEN (data->>'reorderLevel')::NUMERIC ELSE NULL END
) STORED;
...
ALTER TABLE stock_transactions ADD COLUMN tx_date DATE GENERATED ALWAYS AS (
  CASE WHEN data->>'date' ~ '^\d{4}-\d{2}-\d{2}' THEN (data->>'date')::DATE ELSE NULL END
) STORED;
ALTER TABLE stock_transactions ADD COLUMN qty NUMERIC GENERATED ALWAYS AS (
  CASE WHEN data->>'qty' ~ '^-?[0-9]+(\.[0-9]+)?$' THEN (data->>'qty')::NUMERIC ELSE NULL END
) STORED;
