...
async function _spResolveListId(token, listName) {
  ...
  if (!found) {
    ...
    if (cfg && cfg.autoCreate) {
      ...
      // Build the full column set every push writes. Missing any one of them
      // makes Graph 400 the whole write, so this must match _spPushListData.
      await _spEnsureWriteColumns(token, created.id, cfg);
      return created.id;
    }
...
// Columns every _spPushListData write names. A list missing any of them makes
// Graph reject the entire write with a 400, so an auto-created list must have
// the full set before its first push.
function _spWriteColumnsFor(cfg) {
  const cols = [
    { spCol: 'DataBlob',  spType: 'Note'     },
    { spCol: 'UpdatedBy', spType: 'Text'     },
    { spCol: 'UpdatedAt', spType: 'DateTime' },
  ];
  if (cfg.idField) cols.push({ spCol: cfg.idField, spType: 'Text' });
  if (cfg.hasProject) cols.push({ spCol: 'ProjectId', spType: 'Text' });
  if (cfg.indexCols) cols.push(...cfg.indexCols);
  return cols;
}

// Create any of `needed` that the list doesn't already have. Safe to re-run:
// existing columns are skipped, so this also repairs a partially-built list.
async function _spCreateMissingColumns(token, listId, needed) {
  ...
  for (const col of needed) {
    ...
    if (col.spType === 'DateTime')      colDef.dateTime = { format: 'dateTime' };
    else if (col.spType === 'Boolean')  colDef.boolean = {};
    else if (col.spType === 'Note')     colDef.text = { allowMultipleLines: true };
    else                                colDef.text = { allowMultipleLines: false, maxLength: 255 };
    try {
      await fetch(baseUrl, {
        method: 'POST',
        headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
    ...
    } catch (e) { /* non-fatal — reported by the write that needs it */ }
  }
}

async function _spEnsureWriteColumns(token, listId, cfg) {
  return _spCreateMissingColumns(token, listId, _spWriteColumnsFor(cfg));
}
...
async function _spEnsureIndexColumns(token, listName) {
  ...
  // Auto-created lists never went through the setup wizard, so their base write
  // columns (DataBlob/UpdatedBy/UpdatedAt/idField) are ensured here too. This
  // repairs a list left incomplete by an earlier build.
  if (cfg.autoCreate) needed.push(..._spWriteColumnsFor(cfg));
  if (!needed.length) { _spEnsuredCols.add(listName); return; }

  const listId = await _spResolveListId(token, listName);
  // Dedupe — ProjectId/idField can arrive from more than one source above
  const seen = new Set();
  const unique = needed.filter(c => seen.has(c.spCol) ? false : (seen.add(c.spCol), true));
  await _spCreateMissingColumns(token, listId, unique);
...
// ── Build the SHIC_AppData main-blob payload ─────────────────
// When sub-lists push cleanly, the main blob carries only what has
// no dedicated list: settings, tombstones, and the schema stamp.
// When sub-lists fail, we fall back to the legacy full-shape blob
// so SharePoint still holds the data.
function _spBuildMainBlobPayload(subListsOk) {
  if (subListsOk) {
    return {
      settings:      AppState.data.settings || {},
      _deletedIds:   _spDeletedIds,
      _schemaVersion: (typeof SHIC_SCHEMA_VERSION !== 'undefined') ? SHIC_SCHEMA_VERSION : 0,
    };
  }
  // Fallback: sub-lists failed — keep legacy full-shape blob so SP has the data
...
async function spPushData(silent = false) {
  ...
  try {
    ...
    spSetStatus('syncing', 'Uploading to SharePoint...');
    const dataForPush = _spBuildMainBlobPayload(subListsOk);
...
// ── Set admin status ────────────────────────────────────────
// Returns true when the change was written. Callers own the toast and
// re-render so this works from both Settings and User Management.
async function m365SetAdmin(uid, isAdmin) {
  try {
    const token = await getM365AuthToken();
    if (!token) { showToast('Not signed in to Microsoft 365', 'error'); return false; }
    const { siteId } = await spResolveSiteAndList(token);
    const { listId } = await _getOrCreateUsersListId(token, siteId);
    const res = await fetch(
      `https://graph.microsoft.com/v1.0/sites/${siteId}/lists/${listId}/items?$expand=fields&$top=999`,
    ...
    if (!upd.ok) {
      ...
      showToast('Failed to update admin status (' + upd.status + ')', 'error');
      console.warn('[Users] IsAdmin PATCH rejected:', body.slice(0, 200));
      return false;
    }
    return true;
  } catch(e) { showToast('Failed: ' + e.message, 'error'); return false; }
}
// Legacy alias — older call sites use this name.
const m365SetUserAdmin = m365SetAdmin;
...
async function showAuditLog() {
  ...
  try {
    ...
    // No $orderby — SharePoint rejects sorting on a non-indexed field with a
    // 400. Pull newest-first by the built-in created date and sort locally.
    const res = await fetch(`https://graph.microsoft.com/v1.0/sites/${siteId}/lists/${listId}/items?$expand=fields&$orderby=createdDateTime desc&$top=100`, { headers: { Authorization: 'Bearer ' + token } });
    const data = await res.json();
    const items = (data.value||[]).slice().sort((a,b) =>
      String(b.fields?.Title||'').localeCompare(String(a.fields?.Title||'')));
