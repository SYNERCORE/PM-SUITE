  ...
  <div class="card" style="grid-column:1/-1;border:1px solid rgba(56,139,253,.25)">
    ...
    <div style="display:grid;grid-template-columns:1fr auto;gap:8px;align-items:end">
      <div class="form-group" style="margin:0">
        ...
        <input class="form-input" id="localSrvUrl" placeholder="https://procmaster.local" value="${(_dev.localServerUrl||'').replace(/"/g,'&quot;')}">
      </div>
    ...
    <div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)">
      ...
      ${[['projects','Projects'],['tasks','Tasks'],['resources','Resources'],['procurement','Procurement'],['costs','Costs'],['qaqc','QA/QC'],['risks','Risks'],['actions','Action Items'],['documents','Documents'],['stockTransactions','Stock Transactions'],['resourceAllocations','Resource Allocations'],['resourceUsageLogs','Usage Logs (Issue/Return)'],['manpower','Manpower'],['procurementLogs','Procurement Logs'],['issuanceRequests','Issuance Requests'],['equipment','Equipment'],['tools','Tools'],['vehicles','Vehicles'],['consumables','Consumables'],['materials','Materials'],['warehouseLocations','Warehouse Locations'],['thirdParty','Third Party'],['trades','Trades'],['businessUnits','Business Units'],['projectTeam','Project Team'],['dailyMeetingLogs','Daily Meeting Logs'],['progress','Progress'],['kpiData','KPI Data'],['calendar','Calendar'],['assetHistory','Asset History'],['assetUtilization','Asset Utilization'],['libraryDocs','Library Docs']].map(([ent,label]) => `
      ...
      <label style="display:flex;align-items:center;gap:8px;font-size:12px;padding:10px 0 4px;border-top:1px dashed var(--border);margin-top:10px">
        <input type="checkbox" id="apiForceSP" ${_dev.forceSharepointMode?'checked':''} onchange="_apiToggleForceSP(this.checked)">
...
function _localSrvSave() {
  ...
  setDeviceSetting('localServerUrl', raw);
...
function _apiToggleEntity(entity, on) {
  const list = new Set(getDeviceSettings().apiEntities || []);
  if (on) list.add(entity); else list.delete(entity);
  setDeviceSetting('apiEntities', Array.from(list));
...
(function _localSrvBoot() {
  try {
    const url = getDeviceSettings().localServerUrl;
