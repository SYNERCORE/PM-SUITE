...
// Opens the camera scanner modal. Result routes through the callback
// keyed at _whScanCallbacks[<key>]. Existing callers pass a line index;
// generic callers register their own callback and pass its key.
let _whScanCallbacks={};
async function _whScanBarcode(lineIdx,callbackKey){
  // Legacy contract: numeric lineIdx = apply to Receive line by that index
  const key = callbackKey || ('rcv:'+lineIdx);
  if (!callbackKey) _whScanCallbacks[key] = code => _whApplyScan(lineIdx, code);
  // Build modal
  const modalHtml=`
  <div class="modal-overlay open" data-dynamic="1" id="whBarcodeModal" data-key="${key}">
      ...
      <div class="modal-body" style="padding:16px">
        ...
        <div style="display:flex;gap:6px">
          <input id="wh-scan-manual" class="form-control" placeholder="Type or paste barcode…" style="flex:1"
            onkeydown="if(event.key==='Enter')_whScanManual()">
          <button class="btn btn-primary btn-sm" onclick="_whScanManual()">Find</button>
        </div>
      </div>
    </div>
  </div>`;
...
function _whScanManual(){
  const val=(document.getElementById('wh-scan-manual')?.value||'').trim();
  if(!val){showToast('Enter a barcode value','warning');return;}
  const key=document.getElementById('whBarcodeModal')?.dataset?.key||'';
  _whStopScan();
  _whDispatchScan(key,val);
}

function _whDispatchScan(key,code){
  const cb=_whScanCallbacks[key];
  if(cb){
    try{cb(code);}catch(e){console.warn('[wh-scan] callback failed:',e);}
    delete _whScanCallbacks[key];
  }
}
...
function _whScanForIssue(lineIdx){
  ...
  _whScanBarcode(lineIdx,key);
}

// Scan target: assign the scanned code to the item form's barcode field
function _whScanForItemForm(){
  const key='item-barcode';
  _whScanCallbacks[key]=code=>{
    const fld=document.getElementById('wh-add-barcode');
    if(!fld){showToast('Barcode field not found','warning');return;}
    fld.value=code;
    fld.dispatchEvent(new Event('input',{bubbles:true}));
    showToast(`Barcode captured: ${code}`,'success');
  };
  _whScanBarcode(null,key);
}
...
function _whIssLinesHTML(){
  ...
  const rows=_whIssLines.map((ln,idx)=>{
    ...
    return`<tr>
      <td style="padding:5px 4px">
        <div style="display:flex;gap:4px">
          <select class="form-control" style="font-size:12px;flex:1;min-width:0" onchange="_whIssLines[${idx}].itemId=this.value;_whRefreshIss()">
            <option value="">— Select Item —</option>${itemOpts}
          </select>
          <button class="btn btn-secondary btn-sm" style="padding:3px 7px" title="Scan barcode" onclick="_whScanForIssue(${idx})"><i class="fas fa-barcode"></i></button>
        </div>
      </td>
      ...
      <div class="modal-body">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          ...
          <div class="form-group">
            ...
            <div style="display:flex;gap:6px">
              <input class="form-control" id="wh-add-barcode" placeholder="e.g. 4806512345678" value="${existing?.barcode||''}" style="flex:1;min-width:0">
              <button type="button" class="btn btn-secondary btn-sm" title="Scan with camera" onclick="_whScanForItemForm()"><i class="fas fa-barcode"></i></button>
            </div>
          </div>
          <div class="form-group">
